...
<?php
	...
	if(!empty($_GET['action']) && !empty($_GET['hideID'])){
		...
		if($_GET['action']=="delete"){
			$sql_delete = "delete FROM email_log where no = '$ID'";
			$result_delete = mysqli_query($conn, $sql_delete);
			echo "<script language='javascript'>location.href='sysEmaillog.php';</script>";
		} else if($_GET['action']=="modify"){
			$sql_max = "SELECT * FROM email_log where no = '$ID'";
  ...
  <div class="container">
    <div class="row">
      <div class="col-lg-12 page-header">
        <h2><?= translate("編輯郵件備份") ?></h2>
      </div>
    </div>
    <form action="sysEmaillog_edit2.php?hideID=<?=$_GET['hideID']; ?>"  method="post" enctype="multipart/form-data" name="main_form">
    ...
    <table class="table table-hover">
        <tbody>
          <tr>
            ...
            <td class="text-center"><input type="text" class="form-control" id="title" name="title" value="<?php echo $row_max[1] ; ?>" placeholder="<?= translate("標題") ?>"></td>
          </tr>
          <tr>
            <td class="text-center"><?= translate("收件信箱") ?></td>
            <td class="text-center"><input type="text" class="form-control" id="mail" name="mail" value="<?php echo $row_max[2] ; ?>" placeholder="<?= translate("收件信箱") ?>"></td>
          </tr>
          <tr>
          ...
          <tr>
            <td class="text-center"><?= translate("寄送時間") ?></td>
            <td class="text-center"><input type="text" class="form-control" id="send_time" name="send_time" value="<?php echo $row_max[4] ; ?>" placeholder="<?= translate("寄送時間") ?>"></td>
          </tr>

  ...
  $result_ref_paper1 = mysqli_query($conn, $sql_ref_paper);
	...
	if(!empty($_GET['action']) && !empty($_GET['hideID'])){
		...
		if($_GET['action']=="delete"){
			$sql_delete = "delete FROM reviewer where user_no = '$ID'";
			$result_delete = mysqli_query($conn, $sql_delete);
			echo "<script language='javascript'>location.href='reviewer.php';</script>";
		} else if($_GET['action']=="modify"){
			$sql_max = "SELECT * FROM reviewer where user_no = '$ID'";
  ...
  <div class="container">
    <div class="row">
      <div class="col-lg-12 page-header">
        <h2><?= translate("編輯審查委員名單") ?></h2>
      </div>
    </div>
    <form action="reviewer_edit2.php?hideID=<?=$_GET['hideID']; ?>"  method="post" enctype="multipart/form-data" name="main_form">
    ...
    <table class="table table-hover">
        ...
        <tr style="display: none">
            <td class="text-center"><?= translate("序號") ?></td>
            <td class="text-center"><input type="text" class="form-control" id="user_no" name="user_no" value="<?php echo $row_max[0] ; ?>" placeholder="<?= translate("序號") ?>"></td>
          </tr>
          <tr>
            <td class="text-center"><?= translate("帳號(Email)") ?></td>
            <td class="text-center"><input type="text" class="form-control" id="id" name="id" value="<?php echo $row_max[1] ; ?>" placeholder="<?= translate("帳號(Email)") ?>"></td>
          </tr>
          <tr>
            <td class="text-center"><?= translate("姓名") ?></td>
            <td class="text-center"><input type="text" class="form-control" id="name" name="name" value="<?php echo $row_max[2] ; ?>" placeholder="<?= translate("姓名") ?>"></td>
          </tr>
          <tr>
            <td class="text-center"><?= translate("單位") ?></td>
            <td class="text-center"><input type="text" class="form-control" id="institute" name="institute" value="<?php echo $row_max[3] ; ?>" placeholder="<?= translate("單位") ?>"></td>
          </tr>
           <tr>
            <td class="text-center"><?= translate("職稱") ?></td>
            <td class="text-center"><input type="text" class="form-control" id="title" name="title" value="<?php echo $row_max[4] ; ?>" placeholder="<?= translate("職稱") ?>"></td>
          </tr>
          <tr>
            <td class="text-center"><?= translate("領域") ?></td>
            <!--<td class="text-center"><input type="text" class="form-control" id="topic" name="topic" value="<?php echo $row_max[5] ; ?>" placeholder="請輸入領域"></td>
          -->
						<td>
							<select class="selectpicker" name="topic" id="topic">
              <?php
									$i = 0;
									while($row1 = @mysqli_fetch_object($result_ref_paper1))
									{
										$i = $i +1;
									?>
                    <option value="<?=$row1->id; ?>" <?php if ($row_max[5] == $row1->id) echo ' selected="selected"'; ?>  ><?=$row1->name; ?></option>
                  <?php
									}
									?>
              </select>
          ...
          <tr>
            <td class="text-center"><?= translate("是否為Chief") ?></td>
			<td>
				<select class="selectpicker" name="chief" id="chief">
					<option value="Y" <?php if ($row_max[6] == 'Y') echo ' selected="selected"'; ?>  ><?= translate("是") ?></option>
					<option value="N" <?php if ($row_max[6] == 'N') echo ' selected="selected"'; ?>  ><?= translate("否") ?></option>
				</select>
			</td>
          </tr>
          <tr>
            <td class="text-center"><?= translate("已審篇數") ?></td>
            <td class="text-center"><input readonly type="text" class="form-control" id="reviewed_papers" name="reviewed_papers" value="<?php echo $row_max[7] ; ?>" placeholder="<?= translate("已審篇數") ?>" ></td>
          </tr>
          <tr style="display: none">
            <td class="text-center"><?= translate("已寄") ?></td>
						<td>
							<select class="selectpicker" name="mail_sent" id="mail_sent">
								<option value="Y" <?php if ($row_max[8] == 'Y') echo ' selected="selected"'; ?>  ><?= translate("是") ?></option>
								<option value="N" <?php if ($row_max[8] == 'N') echo ' selected="selected"'; ?>  ><?= translate("否") ?></option>
							</select>
						</td>
          </tr>
        </tbody>

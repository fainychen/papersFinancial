  ...
  include("SQLconnect.php");
	if(!empty($_GET['action']) && !empty($_GET['hideID'])){
		...
		if($_GET['action']=="delete"){
			...
			$sql_max = "SELECT * FROM admin where id = '$ID'";
  ...
  <div class="container">
    <div class="row">
      <div class="col-lg-12 page-header">
        <h2><?= translate("編輯總編輯帳號") ?></h2>
      </div>
    </div>
    <form action="admin_edit2.php?hideID=<?=$_GET['hideID']; ?>"  method="post" enctype="multipart/form-data" name="main_form">
    ...
    <table class="table table-hover">
        <tbody>
          <tr>
            ...
            <td class="text-center"><input readonly type="text" class="form-control" id="id" name="id" placeholder="<?= translate("登入帳號") ?>" value="<?php echo $row_max[0]; ?>"></td>
          </tr>
          <tr>
            <td class="text-center"><?= translate("名稱") ?></td>
            <td class="text-center"><input type="text" class="form-control" id="name" name="name" placeholder="<?= translate("名稱") ?>" value="<?php echo $row_max[1]; ?>"></td>
          </tr>
          <tr>
            <td class="text-center"><?= translate("密碼") ?></td>
            <td class="text-center"><input type="password" class="form-control" id="pwd" name="pwd" placeholder="<?= translate("密碼") ?>" value="<?php echo $row_max[2]; ?>"></td>
          </tr>
          <tr>
            <td class="text-center"><?= translate("E-mail") ?></td>
            <td class="text-center"><input type="text" class="form-control" id="email" name="email" placeholder="<?= translate("E-mail") ?>" value="<?php echo $row_max[3]; ?>"></td>
          </tr>
          <tr>
            <td class="text-center"><?= translate("權限設定") ?></td>
            <td>
				<select class="selectpicker" name="role" id="role">
					<option value="0" <?php if ($row_max[4] == '0') echo ' selected="selected"'; ?>  ><?= translate("作者") ?></option>
					<option value="1" <?php if ($row_max[4] == '1') echo ' selected="selected"'; ?>  ><?= translate("審查委員") ?></option>
					<option value="2" <?php if ($row_max[4] == '2') echo ' selected="selected"'; ?>  ><?= translate("編輯委員") ?></option>
					<option value="3" <?php if ($row_max[4] == '3') echo ' selected="selected"'; ?>  ><?= translate("大會工作人員") ?></option>
					<option value="4" <?php if ($row_max[4] == '4') echo ' selected="selected"'; ?>  ><?= translate("系統管理員") ?></option>
				</select>
			</td>
          </tr>
        </tbody>

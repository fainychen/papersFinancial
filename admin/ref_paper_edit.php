...
<?php
    ...
    $sql = "SELECT * FROM ref_paper;";
    $result = mysqli_query($conn, $sql);
	
	if(!empty($_GET['action']) && !empty($_GET['hideID'])){
		$ID = $_GET['hideID'];
		if($_GET['action']=="delete"){
			$sql_delete = "delete FROM ref_paper where id = '$ID'";
			$result_delete = mysqli_query($conn, $sql_delete);
			echo "<script language='javascript'>location.href='ref_paper.php';</script>";
		} else if($_GET['action']=="modify"){
			$sql_max = "SELECT * FROM ref_paper where id = '$ID'";
  ...
  <div class="container">
    <div class="row">
      <div class="col-lg-12 page-header">
        <h2><?= translate("編輯論文類別") ?></h2>
      </div>
    </div>
    <form action="ref_paper_edit2.php?hideID=<?=$_GET['hideID']; ?>"  method="post" enctype="multipart/form-data" name="main_form">
    ...
    <table class="table table-hover">
        <tbody>
					<tr>
            <td class="text-center"><?= translate("論文主題代碼") ?></td>
            <td class="text-center"><input disabled value="<?php echo $row_max[0] ; ?>" type="text" class="form-control" id="id" name="id" placeholder="<?= translate("論文主題代碼") ?>"></td>
          </tr>
          <tr>
            <td class="text-center"><?= translate("論文主題名稱") ?></td>
            <td class="text-center"><input value="<?php echo $row_max[1] ; ?>" type="text" class="form-control" id="name" name="name" placeholder="<?= translate("論文主題名稱") ?>"></td>
          </tr>
					<tr>
            <td class="text-center"><?= translate("審查委員數") ?></td>
            <td class="text-center"><input value="<?php echo $row_max[2] ; ?>" type="text" class="form-control" id="committee_qty" name="committee_qty" placeholder="<?= translate("審查委員數") ?>"></td>
          </tr>
					<tr>
            <td class="text-center"><?= translate("接受最大篇數") ?></td>
            <td class="text-center"><input value="<?php echo $row_max[3] ; ?>" type="text" class="form-control" id="accept_max" name="accept_max" placeholder="<?= translate("接受最大篇數") ?>"></td>
          </tr>
					<tr>
            <td class="text-center"><?= translate("接受並推薦到期刊最大篇數") ?></td>
            <td class="text-center"><input value="<?php echo $row_max[4] ; ?>" type="text" class="form-control" id="recommend_max" name="recommend_max" placeholder="<?= translate("接受並推薦到期刊最大篇數") ?>"></td>
          </tr>
        </tbody>
     </table>
  </form>

...
<?php
  ...
  $result = mysqli_query($conn, $sql);
	$sql_max = "SELECT MAX(id) as max FROM ref_paper;";
    ...
    <table class="table table-hover">
        <tbody>
          <tr>
            ...
            <td class="text-center"><input type="text" class="form-control" id="id" name="id" placeholder="<?= translate("論文主題代碼") ?>"></td>
          </tr>
          <tr>
            <td class="text-center"><?= translate("論文主題名稱") ?></td>
            <td class="text-center"><input type="text" class="form-control" id="name" name="name" placeholder="<?= translate("論文主題名稱") ?>"></td>
          </tr>
			<tr>
            <td class="text-center"><?= translate("審查委員數") ?></td>
            <td class="text-center"><input type="text" class="form-control" id="committee_qty" name="committee_qty" placeholder="<?= translate("審查委員數") ?>"></td>
          </tr>
			<tr>
            <td class="text-center"><?= translate("接受最大篇數") ?></td>
            <td class="text-center"><input type="text" class="form-control" id="accept_max" name="accept_max" placeholder="<?= translate("接受最大篇數") ?>"></td>
          </tr>
			<tr>
            <td class="text-center"><?= translate("接受並推薦到期刊最大篇數") ?></td>
            <td class="text-center"><input type="text" class="form-control" id="recommend_max" name="recommend_max" placeholder="<?= translate("接受並推薦到期刊最大篇數") ?>"></td>
          </tr>
        </tbody>
     </table>
  </form>
...
if($_GET['action']=="save"){
	$id = $_POST['id']; 
  $name = $_POST['name']; 
	$committee_qty = $_POST['committee_qty']; 
  $accept_max = $_POST['accept_max']; 
	$recommend_max = $_POST['recommend_max']; 
	mysqli_query($conn,"insert into ref_paper set 
      id='$id'
      , name='$name'
      , committee_qty='$committee_qty'
      , accept_max='$accept_max'
      , recommend_max='$recommend_max'
			");
	echo "<script language='javascript'> location='ref_paper.php';</script>"; 	
}}
?>
<script>
	function gohome(){

...
<?php
    ...
    $sql = "SELECT * FROM recommended_journal;";
    $result = mysqli_query($conn, $sql);
	
	if(!empty($_GET['action']) && !empty($_GET['hideID'])){
		$ID = $_GET['hideID'];
		if($_GET['action']=="delete"){
			$sql_delete = "delete FROM recommended_journal where id = '$ID'";
			$result_delete = mysqli_query($conn, $sql_delete);
			echo "<script language='javascript'>location.href='recommended_journal.php';</script>";
		} else if($_GET['action']=="modify"){
			$sql_max = "SELECT * FROM recommended_journal where id = '$ID'";
  ...
  <div class="container">
    <div class="row">
      <div class="col-lg-12 page-header">
        <h2><?= translate("編輯推薦期刊") ?></h2>
      </div>
    </div>
    <form action="recommended_journal_edit2.php?hideID=<?=$_GET['hideID']; ?>"  method="post" enctype="multipart/form-data" name="main_form">
    ...
    <table class="table table-hover">
        <tbody>
					<tr>
            <td class="text-center"><?= translate("推薦期刊代碼") ?></td>
            <td class="text-center"><input disabled value="<?php echo $row_max[0] ; ?>" type="text" class="form-control" id="id" name="id" placeholder="<?= translate("推薦期刊代碼") ?>"></td>
          </tr>
          <tr>
            <td class="text-center"><?= translate("推薦期刊名稱") ?></td>
            <td class="text-center"><input value="<?php echo $row_max[1] ; ?>" type="text" class="form-control" id="name" name="name" placeholder="<?= translate("推薦期刊名稱") ?>"></td>
          </tr>
					<tr>
            <td class="text-center"><?= translate("索引") ?></td>
            <td class="text-center"><input value="<?php echo $row_max[2] ; ?>" type="text" class="form-control" id="science_citation_index" name="science_citation_index" placeholder="<?= translate("索引") ?>"></td>
          </tr>
					<tr>
            <td class="text-center"><?= translate("限制條件") ?></td>
            <td class="text-center"><input value="<?php echo $row_max[3] ; ?>" type="text" class="form-control" id="restrictions" name="restrictions" placeholder="<?= translate("限制條件") ?>"></td>
          </tr>
					<tr>
            <td class="text-center"><?= translate("推薦流程") ?></td>
            <td class="text-center">
				<textarea rows="2" type="text" class="form-control" id="recommendation_process" name="recommendation_process"  value="" placeholder="<?= translate("推薦流程") ?>"><?php echo $row_max[4] ; ?></textarea></td>
			</td>
          </tr>
        </tbody>
     </table>
...
<script>
	function gohome(){
		...
		if (answer)
		location.href='recommended_journal.php';
	}
</script>

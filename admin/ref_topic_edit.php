...
<?php
    ...
    $sql = "SELECT * FROM ref_topic;";
    $result = mysqli_query($conn, $sql);
	
	if(!empty($_GET['action']) && !empty($_GET['hideID'])){
		$ID = $_GET['hideID'];
		if($_GET['action']=="delete"){
			$sql_delete = "delete FROM ref_topic where no = '$ID'";
			$result_delete = mysqli_query($conn, $sql_delete);
			echo "<script language='javascript'>location.href='ref_topic.php';</script>";
		} else if($_GET['action']=="modify"){
			$sql_max = "SELECT * FROM ref_topic where no = '$ID'";
...
<nav class="navbar navbar-default">
  ...
  <div class="container">
    ...
    <form action="ref_topic_edit2.php?hideID=<?=$_GET['hideID']; ?>"  method="post" enctype="multipart/form-data" name="main_form">
    ...
    <table class="table table-hover">
        <tbody>
          <tr>
            <td class="text-center"><?= translate("類別代碼") ?></td>
            <td class="text-center"><input readonly type="text" class="form-control" id="no" name="no" value="<?php echo $row_max[0] ; ?>" placeholder="<?= translate("類別代碼") ?>"></td>
          </tr>
          <tr>
            <td class="text-center"><?= translate("類別名稱") ?></td>
            <td class="text-center"><input type="text" class="form-control" id="name" name="name" value="<?php echo $row_max[1] ; ?>" placeholder="<?= translate("類別名稱") ?>"></td>
          </tr>
        </tbody>
     </table>
  </form>

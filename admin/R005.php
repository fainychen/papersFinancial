  ...
  <div class="container">
    <div class="row">
      <div class="col-lg-12 page-header">
        <h2><?= translate("系統參數設定") ?></h2>
      </div>
    </div>
	<button class="btn btn-default "onclick="resetMail()"><?= translate("審查委員通知信-寄送狀態重置") ?></button>
    <br/>
    <br/>
    ...
    <table class="table table-hover">
        <thead>
          <tr>
            <th class="text-center"><?= translate("寄件者名稱") ?></th>
            <th class="text-center"><?= translate("寄件者電子郵件") ?></th>
        ...
        <tbody>
          ...
          <tr>
            ...
            <td class="text-center"><?=$row->sender_email; ?></td>
            <td class="text-center">
							<button class="glyphicon glyphicon-pencil btn btn-default" onclick="self.location.href='R005_edit.php?action=modify&hideID=<?=$row->id; ?>'"></button>
            </td>
          </tr>            
          <?php
            }
...
<script>
 	function resetMail(){
		location.href='R005.php?hideID=reset';
	}; 
</script>
<?php
	if( !empty($_GET['hideID'])){
		$ID = $_GET['hideID'];
		mysqli_query($conn,"update reviewer set mail_sent='N'");
		echo "<script language='javascript'> location='R005.php';</script>";				
	}
?>

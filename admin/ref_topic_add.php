...
<?php
  ...
  $result = mysqli_query($conn, $sql);
	$sql_max = "SELECT MAX(no) as max FROM ref_topic;";
    ...
    <table class="table table-hover">
        <tbody>
          <tr>
            ...
            <td class="text-center"><input type="text" class="form-control" id="no" name="no" placeholder="<?= translate("類別代碼") ?>"></td>
          </tr>
          <tr>
            <td class="text-center"><?= translate("類別名稱") ?></td>
            <td class="text-center"><input type="text" class="form-control" id="name" name="name" placeholder="<?= translate("類別名稱") ?>"></td>
          </tr>
        </tbody>
     </table>
  </form>
...
if($_GET['action']=="save"){
	$no = $_POST['no']; 
  $name = $_POST['name']; 
	mysqli_query($conn,"insert into ref_topic set 
      no='$no'
      , name='$name'
			");
	echo "<script language='javascript'> location='ref_topic.php';</script>"; 	
}}
?>
<script>
	function gohome(){

...
<?php
  ...
  $result = mysqli_query($conn, $sql);
	$sql_max = "SELECT MAX(user_no) as max FROM reviewer;";
    ...
    <table class="table table-hover">
        <tbody>
          <tr style="display: none">
            ...
            <td class="text-center"><input type="text" class="form-control" id="user_no" name="user_no" placeholder="<?= translate("序號") ?>" value="<?php echo $row_max[0] + 1; ?>" ></td>
          </tr>
          <tr>
            <td class="text-center"><?= translate("帳號(Email)") ?></td>
            <td class="text-center"><input type="text" class="form-control" id="id" name="id" placeholder="<?= translate("帳號(Email)") ?>"></td>
          </tr>
          <tr>
          ...
          <tr>
            <td class="text-center"><?= translate("是否為Chief") ?></td>
                ...
                <option value="Y"><?= translate("是") ?></option>
                <option value="N"><?= translate("否") ?></option>
              </select>
            </td>
          </tr>
          <tr>
            <td class="text-center"><?= translate("已審篇數") ?></td>
            <td class="text-center"><input readonly type="text" class="form-control" id="reviewed_papers" name="reviewed_papers" placeholder="<?= translate("已審篇數") ?>" text="0" value="0"></td>
          </tr>
          <tr style="display: none">
...
if($_GET['action']=="save"){
	...
	$topic = $_POST['topic']; 
	$chief = $_POST['chief']; 
	$reviewed_papers = $_POST['reviewed_papers']; 
	$mail_sent = $_POST['mail_sent']; 
	mysqli_query($conn,"insert into reviewer set
			 user_no='$user_no'
			, id='$id'
			, name='$name'
			, institute='$institute'

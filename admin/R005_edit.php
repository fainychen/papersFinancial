...
<?php
	...
	$sql = "SELECT * FROM server_setting;";
	$result = mysqli_query($conn, $sql);
	
	if(!empty($_GET['action']) && !empty($_GET['hideID'])){
		$ID = $_GET['hideID'];
		if($_GET['action']=="delete"){
			$sql_delete = "delete FROM server_setting where id = '$ID'";
			$result_delete = mysqli_query($conn, $sql_delete);
			echo "<script language='javascript'>location.href='R006.php';</script>";
		} else if($_GET['action']=="modify"){
			$sql_max = "SELECT * FROM server_setting where id = '$ID'";
  ...
  <div class="container">
    <div class="row">
      <div class="col-lg-12 page-header">
        <h2><?= translate("編輯系統參數") ?></h2>
      </div>
    </div>
    <form action="R005_edit2.php?hideID=<?=$_GET['hideID']; ?>"  method="post" enctype="multipart/form-data" name="main_form">
    ...
    <table class="table table-hover">
        <tbody>
            ...
            <td class="text-center"><input disabled type="text" value="<?php echo $row_max[0] ; ?>" class="form-control" id="id" name="id" placeholder="<?= translate("編號") ?>"></td>
          </tr>
          <tr>
            <td class="text-center"><?= translate("寄件者名稱") ?></td>
            <td class="text-center"><input type="text" value="<?php echo $row_max[1] ; ?>" class="form-control" id="sender_name" name="sender_name" placeholder="<?= translate("寄件者名稱") ?>"></td>
          </tr>
					<tr>
            <td class="text-center"><?= translate("寄件者電子郵件") ?></td>
            <td class="text-center"><input type="text" value="<?php echo $row_max[2] ; ?>" class="form-control" id="sender_email" name="sender_email" placeholder="<?= translate("寄件者電子郵件") ?>"></td>
					</tr>
					<tr>
            <td class="text-center">server</td>
            <td class="text-center"><input type="text" value="<?php echo $row_max[3] ; ?>" class="form-control" id="server" name="server" placeholder="server"></td>
          </tr>
					<tr>
            <td class="text-center">server_name</td>
            <td class="text-center"><input type="text" value="<?php echo $row_max[4] ; ?>" class="form-control" id="server_name" name="server_name" placeholder="server_name"></td>
					</tr>
					<tr>
            <td class="text-center">server_password</td>
            <td class="text-center"><input type="text" value="<?php echo $row_max[5] ; ?>" class="form-control" id="server_password" name="server_password" placeholder="server_password"></td>
          </tr>
        </tbody>
     </table>
  </form>

...
<?php
    include("SQLconnect.php");
		if ($_COOKIE["role"] == $rEncrypted0 ) {
			$a = $_COOKIE["id"];
			$sql = "SELECT * FROM author where author_no = $a";
		} else {
  ...
  <div class="container">
    <div class="row">
      <div class="col-lg-12 page-header">
        <?php if ($_COOKIE["role"] !== $rEncrypted0 ) { ?>
					<h2><?= translate("作者管理") ?></h2>
				<?php } ?>
				<?php if ($_COOKIE["role"] == $rEncrypted0 ) { ?>
					<h2><?= translate("個人資料維護") ?></h2>
				<?php } ?>
      </div>
    </div>
		<?php if ($_COOKIE["role"] !== $rEncrypted0 ) { ?>
    ...
    <table class="table table-hover">
        <thead>
          <tr>
            <th class="text-center"><?= translate("帳號") ?></th>
        ...
        <tbody>
          ...
          <tr>
            ...
            <td class="text-center">
                <button class="glyphicon glyphicon-pencil btn btn-default" onclick="self.location.href='mb_list_edit.php?action=modify&hideID=<?=base64_encode($encrypted); ?>'"></button>
                <?php if ($_COOKIE["role"] !== $rEncrypted0 ) { ?>
					<button class="btn btn-default glyphicon glyphicon-trash" onclick="deleteitem('<?=$row->author_no; ?>')"></button>
				<?php } ?>
				</td>
          </tr>            
          <?php
...
<script>
	function additem(a){
		location.href='mb_list_add.php';
	}
	function deleteitem(a){
		answer = confirm("<?= translate('您確定要刪除此筆資料嗎？') ?>"	);
		if (answer) {
			location.href='mb_list_edit.php?action=delete&hideID='+a+'';
			//alert(a);
		}
	}
</script>

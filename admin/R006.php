...
<nav class="navbar navbar-default">
  ...
  <div class="container">
    ...
    <table class="table table-hover">
        <thead>
          <tr>
            <th class="text-center"><?= translate("編號") ?></th>
            <th class="text-center"><?= translate("主旨") ?></th>
            <th class="text-center"><?= translate("說明") ?></th>
        ...
        <tbody>
          ...
          <tr>
            ...
            <td class="text-center">
				<button class="glyphicon glyphicon-pencil btn btn-default" onclick="self.location.href='R006_edit.php?action=modify&hideID=<?=$row->id; ?>'"></button>
				<button  style="display: none" class="btn btn-default glyphicon glyphicon-trash" onclick="deleteitem('<?=$row->id; ?>')"></button>
            </td>
          </tr>            
          <?php
            }
...
<script>
	function additem(a){
		location.href='R006_add.php';
	}
	function deleteitem(a){
		answer = confirm("<?= translate('您確定要刪除此筆資料嗎？') ?>"	);
		if (answer) {
			location.href='R006_edit.php?action=delete&hideID='+a+'';
			//alert(a);
		}
	}
</script>

...
<?php
	...
	if(!empty($_GET['action']) && !empty($_GET['hideID'])){
		...
		if($_GET['action']=="delete"){
			$sql_delete = "delete FROM email_template where id = '$ID'";
			$result_delete = mysqli_query($conn, $sql_delete);
			echo "<script language='javascript'>location.href='R006.php';</script>";
		} else if($_GET['action']=="modify"){
			$sql_max = "SELECT * FROM email_template where id = '$ID'";
  ...
  <div class="container">
    <div class="row">
      <div class="col-lg-12 page-header">
        <h2><?= translate("編輯通知信設定") ?></h2>
      </div>
    </div>
    <form action="R006_edit2.php?hideID=<?=$_GET['hideID']; ?>"  method="post" enctype="multipart/form-data" name="main_form">
    ...
    <table class="table table-hover">
		...
		<tr style="display: none">
          ...
          <tr>
            ...
            <td class="text-center"><input type="text" value="<?php echo $row_max[1] ; ?>" class="form-control" id="description" name="description" placeholder="<?= translate("說明") ?>"></td>
          </tr>
		<tr>
            <td class="text-center"><?= translate("主旨") ?></td>
            <td class="text-center"><input type="text" value="<?php echo $row_max[2] ; ?>" class="form-control" id="title" name="title" placeholder="<?= translate("主旨") ?>"></td>
          </tr>
			<tr>
            <td class="text-center"><?= translate("內容") ?></td>
            <td class="text-center">
            	<textarea cols="140" rows="36" id="content" name="content" ><?php echo $row_max[3] ; ?></textarea>
            </td>
		</tr>
			<tr>
            <td class="text-center"><?= translate("備註") ?></td>
            <td class="text-center"><input type="text" value="<?php echo $row_max[4] ; ?>" class="form-control" id="note" name="note" placeholder="<?= translate("備註") ?>"></td>
          </tr>
			<tr>
            <td class="text-center"><?= translate("說明") ?></td>
            <td>
